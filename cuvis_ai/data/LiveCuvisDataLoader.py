...
class LiveCuvisDataLoader(BaseDataSet):
    ...
    def __init__(self, path: Optional[Union[str, Path]] = None,
        ...
        super().__init__(path, transforms, transform,
                         target_transform, output_format, output_lambda)
        self._clear()
    ...
    def initialize(self, path: Union[str, Path], simulate: bool = False, force: bool = False, timeout_s: int = 20) -> bool:
        ...
        if self.initialized:
            if force:
                ...
                raise RuntimeError(
                    "Cannot initialize an already initialized LiveCuvisData. Use force=True if this was intended.")
        self.root = path
        ...
        if self.sess is not None:
            self.camera = cuvis.AcquisitionContext(
                self.sess, simulate=simulate)
    ...
    def processing_mode(self, val: cuvis.ProcessingMode):
        ...
        if val == cuvis.ProcessingMode.Preview:
            raise ValueError(
                "Processing mode 'Preview' is not supported, as it does not produce any cubes. Use 'cuvis.ProcessingMode.Raw' instead.")
    ...
    def _fetch_mesu(self) -> cuvis.Measurement:
        ...
        while mesu is None:
            try:
                if self.continuous:
                    mesu = self.camera.get_next_measurement(int(
                        self.capture_timeout_ms))
                else:
                    mesu = self.camera.capture_at(int(self.capture_timeout_ms))
            except SDKException:
                raise RuntimeError(
                    "Could not fetch a measurement from the device - Timeout")
        return mesu
    ...
    def _mesu2TensorAndTransform(self, mesu: cuvis.Measurement, astype: np.dtype) -> tv_tensors.Image:
        ...
        cube = self._apply_transform(cube.permute(
    ...
    def _capture(self):
        ...
        for mesu in mesus:
            ...
            for reftype, refmesu in self._refcache.items():
                meta["references"][reftype] = self._mesu2TensorAndTransform(
                    refmesu, self.provide_datatype)
    ...
    def load(self, params: Dict, filepath: str):
        ...
        self.processing_contextessing_mode = cuvis.ProcessingMode(
